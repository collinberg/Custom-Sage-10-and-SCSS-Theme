...
export class Module extends Service {
    ...
    async resolve(signifier, context) {
        try {
            ...
            this.logger.info(chalk.dim(`resolved ${signifier} to ${this.app.root.relPath(normalpath)}`));
            return normalpath;
        }
        catch (err) {
    ...
    async tryImport(signifier, context) {
        try {
            ...
            this.logger.info(chalk.dim(`${signifier} could not be imported (optional)`), err);
        }
    }
    /**
     * Make context URL
...
__decorate([
    ...
    __metadata("design:returntype", URL)
